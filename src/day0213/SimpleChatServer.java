...
//		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
	...
	private void openServer() throws IOException {
		...
		// 4. 메시지를 읽기 위한 스트림과 메시지를 보내기 위한 스트림을 소켓에서 연결
...
//		readMsg();
		// 내 클래스와 has a 관계로 Thread 생성
		Thread th = new Thread(this);
		th.start();	// => run() 호출
	}	// openServer
	
	/**
	 * 연결된 스트림을 사용하여 메시지를 무한 루프로 읽어 들여 J.T.A에 출력
	 */
	@Override
	public void run() {
		// Thread로 코드가 동작 (동시에 일 처리)
		if(disReadStream != null) {
			String msg = ""	;
			try {
				while(true) {
...
//	}	// readMsg
	...
	/**
	 * 스트림에 메시지를 써서 접속자 소켓(목적지)으로 보내는 일
	 * @throws IOException 
	 */
